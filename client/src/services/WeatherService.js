...
// const OPEN_WEATHER_API_KEY = process.env.OPEN_WEATHER_API_KEY || 'b714ec74bbab5650795063cb0fdf5fbe';
const OPEN_WEATHER_IMG_URL = 'http://openweathermap.org/img/w';

const getWeather = (urlPos) => {
    return new Promise((resolve, reject) => {
        API.getWeather(urlPos)
            // .get(url)
            .then(response => {
                if (response ) {
...
class WeatherService {
    ...
    getCurrentWeatherByPosition({latitude, longitude}) {
        ...
        return getWeather({lat:latitude, lng:longitude});
    }
    ...
    getDailyWeatherByPosition({latitude, longitude}) {
        ...
        return getDailyWeather({lat:latitude, lng:longitude});
    }
    ...
    getHourlyWeatherByPosition({latitude, longitude}) {
        ...
        return getHourlyWeather({lat:latitude, lng:longitude});
    }
}
